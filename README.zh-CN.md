...
- 会话:`--continue`、`--resume 文件`、`/session`(新建、分叉、恢复);`clari sessions` 列出会话,`clari sessions prune --older-than 30d` 或 `--keep N` 连同 trace 与 MCP 旁车一起删,加 `--yes` 才真删;一次性模式 `clari once "任务" --json`(策略 A/B 的执行器)
