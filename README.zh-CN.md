...
- 工具描述三套风格可选(guided / terse / strict),`--tool-prompts` 或 `/toolprompts` 切换,逐工具在编辑器里改并写回配置;模型只看到描述文本
...
- 会话:`--continue`、`--resume 文件`、`/fork`;`clari sessions` 列出会话,`clari sessions prune --older-than 30d` 或 `--keep N` 连同 trace 与 MCP 旁车一起删,加 `--yes` 才真删;一次性模式 `clari once "任务" --json`(策略 A/B 的执行器)
