...
export const SETUP_SECTIONS: readonly SetupSection[] = [
  ...
  },
  {
    id: "recording",
    title: "Recording",
    description: "The evidence available when you inspect a request.",
    keys: ["saveInputs"],
    action: { label: "Open request inspector", command: "/inspect requests" },
  },
];

type Guide = { title: string; effect: string; reason: string };
export const SETUP_GUIDE: Readonly<Record<string, Guide>> = {
  ...
  "prompt.skills.list": {
    ...
    effect:
      "Controls whether the model sees discovered skill names. With none, you can still invoke a skill yourself.",
    reason: "List skills so the model can discover available procedures. Listing adds prompt text.",
  },
  "prompt.skills.load": {
    title: "Skill loading",
    effect: "Lets the model read a skill file directly or load it through a dedicated tool.",
    reason:
  ...
  compaction: {
    ...
    effect:
      "Summary replaces older history with model-written text. Clear removes old tool results. Pipeline does both.",
    reason:
