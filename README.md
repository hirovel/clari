...
- MCP 客户端(零依赖手写,stdio 与 Streamable HTTP,2026-07-28 无状态协议与旧版握手都认):配置 `mcp.servers` 或项目根 `.mcp.json`,工具名 `mcp__server__tool`,审批规则 `mcp:server:tool`,每次 JSON-RPC 往返都是事件,`/mcp` 看状态
